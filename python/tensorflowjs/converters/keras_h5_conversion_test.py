...
class ConvertH5WeightsTest(unittest.TestCase):
  ...
  def testTranslateBatchNormalizationV1ClassName(self):
    ...
    out = conversion.translate_class_names(json_object)
    # Some class names should not have been changed be translate_class_names().
    self.assertEqual(out['class_name'], 'Sequential')
    self.assertEqual(out['keras_version'], '2.2.4-tf')
    self.assertEqual(out['config']['layers'][0]['class_name'], 'Dense')
    # The translation should have happend:
    #     BatchNormalizationV1 --> BatchNormalization.
    self.assertEqual(
        out['config']['layers'][1]['class_name'], 'BatchNormalization')
    self.assertEqual(out['config']['layers'][2]['class_name'], 'Dense')

    # Assert that converted JSON can be reconstituted as a model object.
    model = keras.models.model_from_json(json.dumps(out))
  ...
  def testTranslateUnifiedGRUAndLSTMClassName(self):
    ...
    #     UnifiedGRU --> GRU.
    #     UnifiedLSTM --> LSTM.
    self.assertEqual(out['config']['layers'][0]['class_name'], 'GRU')
    self.assertEqual(out['config']['layers'][1]['class_name'], 'LSTM')
