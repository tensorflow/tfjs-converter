...
def _createKerasModel(layer_name_prefix, h5_path=None):
  ...
  output = keras.layers.Dense(
      2, use_bias=False,
      kernel_initializer='ones', name=layer_name_prefix + '2')(dense1)
...
class APIAndShellTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    if os.path.isdir(os.path.join(os.path.dirname(__file__), 'tensorflowjs')):
  ...
  def testSaveKerasModel(self):
    ...
    json_content = json.load(open(os.path.join(self._tmp_dir, 'model.json')))
    ...
    self.assertEqual(
        ...
        sorted([
            'MergedDense1/kernel', 'MergedDense1/bias', 'MergedDense2/kernel']))
  ...
  def testConvertTensorFlowSavedModel(self):
    ...
    tfjs.converters.convert_tf_saved_model(
        'a/Softmax', output_dir, 'serve', self.tf_saved_model_dir)
    ...
    with open(os.path.join(output_dir, 'weights_manifest.json'), 'rt') as f:
      output_json = json.load(f)
    self.assertEqual(output_json, weights)

    # Check the content of the output directory.
    self.assertTrue(glob.glob(
        os.path.join(output_dir, 'tensorflowjs_model.pb')))
    self.assertTrue(glob.glob(
  ...
  def testInvalidInputFormatRaisesError(self):
    process = subprocess.Popen(
        ['tensorflowjs_converter', '--input_format', 'nonsensical_format',
         self._tmp_dir, self._tmp_dir],
        stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    _, stderr = process.communicate()
    self.assertGreater(process.returncode, 0)
    self.assertIn(b'--input_format', tf.compat.as_bytes(stderr))
  ...
  def testConvertTFSavedModelWithCommandLineWorks(self):
    ...
    # Load the saved weights as a JSON string.
    output_json = json.load(open(
        os.path.join(output_dir, 'weights_manifest.json'), 'rt'))
    self.assertEqual(output_json, weights)
    ...
    self.assertTrue(
        glob.glob(os.path.join(output_dir, 'group*-*')))
