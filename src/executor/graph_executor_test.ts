  ...
  describe('utility functions', () => {
    describe('checkOutput', () => {
      ...
      it('should reject missing outputs', () => {
        expect(() => executor["checkOutput"]([outputNode], ["missing"]))
            .toThrow(new Error(
      ...
      it('should reject missing outputs with child tensors', () => {
        expect(() => executor["checkOutput"]([outputNode], ["missing:0"]))
            .toThrow(new Error(
              `The following outputs are not generated by the execution: ` +
              `[missing].`));
      });

      it('should accept matching outputs', (done) => {
        executor["checkOutput"]([outputNode], ["output"]);
        done();
      });

      it('should accept matching outputs with child tensors', (done) => {
        executor["checkOutput"]([outputNode], ["output:0"]);
        done();
      });
