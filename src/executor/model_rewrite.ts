...
export function foldConv2dPrelu(graph: Graph, weightMap: NamedTensorsMap) {
  for (const key in graph.nodes) {
    const addNode = graph.nodes[key];
    if (addNode == null || addNode.op !== 'Add' && addNode.op !== 'AddV2' ||
        addNode.inputNames.length !== 2) {
    ...
    const preluNode = addNode.inputs.find(input => input.op === 'Prelu');
    if (preluNode == null || preluNode.op !== 'Prelu' ||
        preluNode.inputNames.length !== 2) {
      continue;
    }

    const fusedConv2D =
        addNode.inputs.find(input => input.op === '_FusedConv2D');
    if (fusedConv2D == null) {
      continue;
    }
    const fusedOps = getParamValue(
                         'fusedOps', fusedConv2D, weightMap,
                         new ExecutionContext(weightMap, {})) as string[];

    if (fusedOps[0] !== 'biasadd' || fusedOps.length > 1) {
      continue;
    }

    const alphaNode = preluNode.inputs.find(input => input.op === 'Const');
    if (alphaNode == null) {
      continue;
    }

    fusedOps.push('prelu');
    fusedConv2D.inputNames.push(alphaNode.name);
    fusedConv2D.inputs.push(alphaNode);

    const outputNodes = preluNode.children;

    // Clean up the children and inputs of input/output nodes of the subgraph.
    const preluIndex = fusedConv2D.children.indexOf(preluNode);
    if (preluIndex > -1) {
      fusedConv2D.children.splice(preluIndex, 1);
    }

    outputNodes.forEach(node => {
      const preluIndex = node.inputNames.indexOf(preluNode.name);
      if (preluIndex > -1) {
        node.inputNames[preluIndex] = fusedConv2D.name;
        node.inputs[preluIndex] = fusedConv2D;
      }
      fusedConv2D.children.push(node);
    });

    // The fusedConv2D node should be an output node.
    if (outputNodes.length === 0) {
      const preluIndex = graph.outputs.indexOf(preluNode);
      if (preluIndex > -1) {
        graph.outputs.splice(preluIndex, 1);
      }
      if (fusedConv2D.children.length === 0) graph.outputs.push(fusedConv2D);
    }
    // remove the nodes for prelu.
    delete graph.nodes[preluNode.name];
  }
}
