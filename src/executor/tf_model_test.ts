...
let model: TFModel;
const bias = tf.tensor1d([1], 'int32');
const WEIGHT_MAP = {
  'Const': bias
};
...
describe('Model', () => {
  beforeEach(() => {
    ...
    spyOn(tf, 'loadWeights').and.returnValue(weightPromise);
  ...
  describe('eval', () => {
    it('should generate the output', async () => {
      await model.load();
      const input = tf.tensor1d([1], 'int32');
      const output = model.eval({'Input': input}, 'Add');
      expect((output as tf.Tensor).dataSync()[0]).toEqual(2);
    });
  });
