...
export let executeOp: OpExecutor = (node: Node, tensorMap: NamedTensorsMap):
  ...
  switch (node.op) {
    case 'conv1d': {
      ...
      return [tf.conv1d(
          getParamValue('x', node, tensorMap) as tf.Tensor3D,
          getParamValue('filter', node, tensorMap) as tf.Tensor3D, stride,
          pad as 'valid' | 'same')];
    }
    case 'conv2d': {
      ...
      return [tf.conv2d(
          ...
          getParamValue('filter', node, tensorMap) as tf.Tensor4D,
    ...
    case 'conv2dTranspose': {
      ...
      const stride = getParamValue('strides', node, tensorMap) as number[];
      const pad = getParamValue('pad', node, tensorMap);
      return [tf.conv2dTranspose(
          getParamValue('x', node, tensorMap) as tf.Tensor3D | tf.Tensor4D,
          getParamValue('filter', node, tensorMap) as tf.Tensor4D, shape,
          [stride[1], stride[2]], pad as 'valid' | 'same')];
    }
    case 'depthwiseConv2d': {
      ...
      return [tf.depthwiseConv2d(
          getParamValue('input', node, tensorMap) as tf.Tensor3D | tf.Tensor4D,
          getParamValue('filter', node, tensorMap) as tf.Tensor4D,
          [stride[1], stride[2]], pad as 'valid' | 'same',
          [rates[0], rates[1]])];
    }
